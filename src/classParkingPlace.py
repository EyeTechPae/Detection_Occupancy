...
#Mask: path to ParkingPlace mask
    ...
    def __init__(self, mask, ID, threshold, occupied, alpha):
    ...
    def setMask(self, frame_path):
        ...
        self.kp,self.des=occ.computeSurfFeatures(self.actualMask, self.alpha)
    ...
    def checkOccupancy(self, frame):
        ...
        kp, des= occ.computeSurfFeatures(frame_masked, self.alpha)
        good_matches = occ.computeGoodMatches(frame_masked, self.alpha, self.kp, self.des)
