...
class Cam4(object):
	...
	def isZoneIn(self): #override 
		...
		ret = True
		str_open, str_dila = camera4.get_strelements(28,53)
		fgmask = fgbg.apply(frame_applied)
		img = camera4.operacions_morfologiques(fgmask, str_open, str_dila)
		contours = camera4.get_contours (img)
		centres, frame_applied = camera4.get_centroids (contours, frame_applied)
		if len(centres)>0:
			return True
		else:
			return False
